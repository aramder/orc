...
| Ref | Value / part | Function | LCSC | Status |
|---|---|---|---|---|
| RB1–RB10, RP1–RP10 | **UNI-ROYAL 4D03WGJ0103T5E**, isolated 4×10kΩ array, 0603x4 (8-pin, 4 independent 2-terminal resistors — confirmed not a bussed/shared-common design) | PCA9555→NPN base (RBn) and gate pull-up to V_COIL_IN (RPn) — 20 discrete 10kΩ resistors total, both banks share one part | C29718 | ✅ **decided 2026-08-01** — confirmed genuinely JLCPCB Basic via a maintained Basic-parts CSV listing (tagged "Very popular"), not just circumstantial stock-depth — the first sourcing pass this session to get a real non-circumstantial tier confirmation. 1.8M+ in stock. 20 resistors → 5 array packages (4 each), real part-count and board-area reduction with no Extended-tier risk. |
...
| D-flyback ×10 (refs TBD) | **BAT54C** (JSCJ), SOT-23-3L dual Schottky common-cathode, only one leg used | Per-channel coil flyback | C2135 | ✅ **decided 2026-08-01, user call** — JSCJ listing, 164,160 in stock, Vr 30V / If 200mA — right-sized for the ~45mA/9V coil duty (vs. SS34's 40V/3A overkill), SOT-23 saves real board area vs. SS34's SMA package across 10 instances. Same part/function as the onsemi C236933 candidate found in the initial sourcing pass, different manufacturer/listing with deeper stock. **Tier not directly confirmed** — hit the same recurring JLCPCB-badge-doesn't-render limit on the direct page, a targeted search, and even JLCPCB's own Basic-parts catalog page (JS-paginated, only rendered an unrelated slice). Circumstantial supporting point: JSCJ is the same manufacturer already used for QN1-10 (MMBT3904/C20526), also treated as circumstantially Basic. Locked per user decision despite the unconfirmed badge — same standard applied to MMBT3904. |

**Open, design-level item**: flyback diode presence on the *relay board itself* is still unconfirmed — needs bench inspection or a continuity/diode check across a coil's harness pins. Whether ORC needs to supply this diode at all is the remaining open question — the donor Motorola board apparently used a per-channel diode alongside the P-ch FET and NPN (user's own observation; PMUN1046A_RE has no component-level schematic to independently confirm this against — its own docs state no netlist was ever found for the URC). If it turns out not to be needed, BAT54C/C2135 above just doesn't get placed — no harm in having it sourced either way.
