...
- **CAN node address, assigned 2026-08-04**: `IO0`/`IO1`/`IO3`/`IO10` (4 of the 9 previously-spare GPIO) are now reserved for a 4-bit configurable CAN node address (0-15), deliberately on the MCU's own Domain A side rather than through the PCA9555 (which lives on the galvanically-isolated Domain B side — address-select logic belongs with the MCU that actually forms CAN arbitration IDs, not across the isolation barrier). Scheme: external 10kΩ pulldown to `GND_A` per bit, switch/jumper to `+3V3_A` per bit (open=0, closed=1), read as plain digital inputs by firmware — see `firmware/lib/orc_can_addr/orc_can_addr.h` for the pin-pick rationale and citations. **Pin pick avoided GPIO2 deliberately**: it's the one remaining unclaimed ESP32-C3 strapping pin (of the full {GPIO2, GPIO8, GPIO9} set); GPIO0/1/3/10 are confirmed non-strapping (Espressif *ESP32-C3 Series Datasheet* v2.4, Table 3-1/3-2/3-3), so unlike GPIO8/9's I2C assignment there's no boot-strap timing window to check for these four at all. **Schematic not yet updated to match** — this sheet's Pass 2 (above) explicitly no-connect-flagged `IO0`/`IO1`/`IO3`/`IO10` as "genuinely spare, no function assigned yet"; those no-connects need removing and the switch/pulldown network needs adding before this is real on the schematic, not just in firmware. `IO2`, `IO4`–`IO7` remain genuinely spare (5 GPIO left after this assignment).
