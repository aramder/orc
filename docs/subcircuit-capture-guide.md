...
| D-flyback ×10 (refs TBD) | Candidate: **BAT54C** (onsemi), SOT-23-3L dual Schottky common-cathode, only one leg used | Per-channel coil flyback | C236933 | ⚠️ sourced live 2026-08-01, not locked. Vr 30V / If 200mA — right-sized for the ~45mA/9V coil duty (vs. SS34's 40V/3A overkill), and SOT-23 vs. SS34's SMA package saves real board area across 10 instances. **Tier unconfirmed** — same recurring JLCPCB-badge-doesn't-render issue; page text showed only generic "Economic and Standard" language, not a confirmed Basic reading. **Do one direct human visual check of the JLCPCB partdetail page before locking this in** — if it confirms Basic, this replaces SS34 in this role; if not, fall back to SS34 reuse below. |

**Open, design-level item**: flyback diode presence on the *relay board itself* is still unconfirmed — needs bench inspection or a continuity/diode check across a coil's harness pins. **If ORC needs to supply flyback diodes per channel**, the choice is between the BAT54C candidate above (smaller, right-sized, tier-unconfirmed) and **reusing SS34** (C8678, already Basic-confirmed, already used ×2 elsewhere as D5/D6 — 40V/3A is far more than the ~45mA/coil duty needs, but SMA package costs more board area at ×10). Whether the diode is needed at all is still the prior open question — the donor Motorola board apparently used a per-channel diode alongside the P-ch FET and NPN (user's own observation; PMUN1046A_RE has no component-level schematic to independently confirm this against — its own docs state no netlist was ever found for the URC).
