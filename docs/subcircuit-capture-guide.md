...
**GPIO reservation for node-ID DIP switch — superseded, see the "CAN node address" entry further down this section** (corrected 2026-08-04, to as-fabbed pins GPIO0–3, not the GPIO0/1/3/10 originally planned when this note was first written 2026-08-02, before the board was sent to fab). Kept here only as a pointer so this paragraph doesn't contradict the later, accurate one.
...
- ~~**U1/U7 — ESP32-S3-WROOM-1U footprint.**~~ **Stale, pre-pivot.** Superseded 2026-08-01/02: the bare ESP32-S3-WROOM-1U + custom USB-C circuit this referred to is gone from the design entirely, replaced by the pre-made ESP32-C3 Super Mini module (see MCU sheet section above). No footprint work needed here — the module's own footprint is the open item, tracked in the MCU section's own "still open" list, not this one.
- ~~**SW2 — GPIO0/BOOT button.**~~ **Stale, pre-pivot, and the pin name would now be actively misleading.** SW1/SW2 were the old custom-circuit's EN/BOOT buttons — explicitly dropped (MCU section: "SW1/SW2 are now redundant — don't build them into the schematic"), since the Super Mini module has its own onboard RST/BOOT buttons. Never built. **Worth flagging explicitly since it shares a pin name with something real**: this stale entry's `GPIO0_A` has no relation to the current `NODE_ID0` net also on GPIO0 (see "CAN node address" further down this section) — that's the module's header-broken GPIO0, doing address-select duty, not a leftover BOOT-button wire. No conflict, just a naming coincidence between an abandoned plan and the real board; left both facts here so a future read doesn't mistake one for the other.
