...
| Diode part count | **Down to 2 distinct types**: SS34 (buck catch diode ×2 — see open item re: whether the now-synchronous LMR50410 needs it at all — plus **10× coil flyback, confirmed needed 2026-08-05**, user inspected the relay board directly and confirmed it has no internal flyback diodes), USBLC6-2SC6 (USB ESD) | Consolidated this pass: CAN ESD dropped (SN65HVD230 has built-in ±16kV bus ESD), input load-dump TVS removed (transient protection now an open item, see below), and both catch diodes plus the flyback role moved from SS56 to a single board-wide SS34 (C8678). Flyback was originally conditional ("if ORC needs to supply it") — resolved: it does. |
...
**Real-hardware fault found and fixed, 2026-08-10 — mechanical clearance, not a schematic problem.** First bench test of a real, populated ORC PCB mounted in its enclosure: relays only weakly/partially actuated (audible faint click, not enough force to close contacts), and the I2C bus to the PCA9555 started throwing driver-level errors mid-run. Root cause, found by physical inspection: **this connector's bare, unshrouded THT pins were shorting against the metal enclosure chassis.** The connector was deliberately chosen unshrouded (see BOM #7's confirmation above) with no enclosure-clearance analysis done against that choice at the time. A short from a coil-drive or chassis-ground pin under load pulled the shared supply rail(s) down hard enough to both starve coil drive current and glitch the PCA9555's own logic supply — one electrical fault producing what initially looked like two separate bugs (weak coils, corrupted I2C). Confirmed NOT a coil-supply sizing/buck problem, NOT a firmware bug — removing the short from the enclosure fixed both symptoms.

**Follow-up needed, not yet done**: this connector choice needs either (a) enclosure clearance/standoff verification before the next assembly goes in a case, (b) an insulating boot/shroud added over the header, or (c) reconsidering the unshrouded choice itself now that it's caused a real fault, not just a theoretical one. Whoever does PCB-to-enclosure fit-up next should treat this as a real, demonstrated risk, not a hypothetical.
...
**Resolved 2026-08-02 — GPIO pins picked, board sent to fab; corrected 2026-08-04 once back at the schematic to match what was actually built.** Superseding the open items below, most of which this closes:
...
- [ ] **Real-hardware confirmation of the GPIO2 boot-safety claim — partially confirmed 2026-08-10.** First live test on the real, powered ORC PCB (`i2c_scanner`, GPIO2 bit at its OPEN/LOW position, all four switches open): boots and runs normally, idle bus reads HIGH/HIGH, CAN address correctly reads `0` (`0000`). This is the predicted-safe outcome from the static Table 3-3 analysis, now empirically observed for real — not just a clean build. **Still open**: only the all-open (GPIO2=LOW) switch position has been tested; the datasheet analysis concluded both positions land in the same safe SPI-Boot table row, but that CLOSED/HIGH position on GPIO2 specifically hasn't been walked yet — do that before closing this item for good.
...
| 3d | Per-channel coil flyback diode (×10) | SS34 | C8678 (same part as D1/catch diode — separate line item, separate qty) | Basic, confirmed | **Added 2026-08-05** — user directly inspected the relay board and confirmed it has no internal flyback diodes, so ORC's control board must supply one per coil. Reuses the board-wide SS34 already qualified for the buck catch-diode role (40V/3A) rather than sourcing a new part; ~9V/45mA-per-coil duty needs a fraction of that rating. Schematic placement (across each P-FET-switched coil output) not yet drawn — see open items. |
...
- [ ] **NEW, real-hardware finding 2026-08-10 — harness connector (BOM #7) enclosure clearance.** Confirmed on the bench: this connector's unshrouded THT pins shorted against the metal enclosure chassis, sagging shared supply rail(s) enough to weaken coil actuation and corrupt I2C to the PCA9555 (see "Harness connector — measured pinout" above for the full writeup). Fixed for this bench session by physical repositioning; **not yet fixed as a design item** — needs enclosure standoff/clearance verification, an insulating boot, or reconsidering the unshrouded connector choice before the next unit goes in a case.
...
- [x] **Flyback diode presence on the relay board — resolved 2026-08-05, user confirmed by direct inspection: the relay board does NOT have internal flyback diodes.** ORC's control board must supply them, one per coil (×10). Per the diode-count consolidation already recorded above (BOM #22/"Diode part count"), this uses the existing board-wide **SS34 (LCSC C8678, 40V/3A Schottky)** — the same part already qualified for the buck catch-diode role — not a new part number. SS34's rating is far above the ~9V/45mA-per-coil flyback duty; the earlier SS56 candidate mentioned in this doc's history was superseded when the board-wide diode consolidation moved to SS34 (see "Diode part count" row and BOM #D1 above). BOM line added (#3d, below). **Follow-up, not yet done**: place the 10× SS34 flyback diodes on the coil-drive schematic sheet, across each P-FET-switched coil output — this item and the BOM line only resolve *whether/what* ORC needs to supply, not the schematic placement itself.
