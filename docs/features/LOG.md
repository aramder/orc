...
>
> **FR-002 resolved 2026-08-31**: it used TPDO2's spare byte-7, not a new
> object-dictionary entry, so it did **not** end up depending on FR-005 —
> confirming the conditional above the way it was written to be confirmed.
...
| FR-002 | Signal relay-hardware faults on the CAN wire, not just Serial | implemented (2026-08-31 — TPDO2 byte-7 chosen over EMCY, no FR-005 dependency needed; not yet compiled/flashed to real hardware, see shard) |
...
| FR-005 | Replace the hard-coded SDO object handler with an object-dictionary table | open (2026-08-29 — split out of FR-003; prerequisite for FR-003 only — FR-002 resolved 2026-08-31 without needing it, see FR-002's row) |
...
| BUG-005 | An unpowered/absent PCA9555 stalls `loop()` to ~0.1 Hz, stretching CAN heartbeat/TPDO2 far past their 1000 ms cadence so the host reads the node as flapping | fixed, verified on real hardware 2026-09-04 (found 2026-08-30 by a rigos-core session, measured 9450 ms heartbeat period against a 1000 ms target; fixed via a probe cooldown in `requireRelayHardware()` plus two smaller reassert/logging changes; real-hardware retest with the DC side disconnected and a 1Hz RPDO1 keepalive showed heartbeat/TPDO2 holding at 1000.0 ms mean gap over 70 s, TPDO1 correctly withheld while absent, automatic recovery within ~0.7 s of DC restore, no reboot needed. Same symptom class as BUG-003, different cause) |
